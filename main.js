...
document.addEventListener('keydown', (event) => {
    if (
        (event.key.toLowerCase() === 'w' || event.key === 'ArrowUp') &&
        state != 3
    ) {
        state = 1;
    } else if (
        (event.key.toLowerCase() === 'a' || event.key === 'ArrowLeft') &&
        state != 4
    ) {
        state = 2;
    } else if (
...
const mainGame = setInterval(() => {
    ...
    let distance = Math.sqrt(
        (leftPositionMainHead - (xFruit + 25)) ** 2 +
            (topPositionMainHead - (yFruit + 25)) ** 2
    );
    if (distance < 50) generateFruit();
...
function distanceFruitOk(xFruitArg, yFruitArg) {
    for (let i = 0; i < body.length; i++) {
        ...
        if (
            Math.sqrt(
                (xFruitArg - bodyNodeXPosition) ** 2 +
                    (yFruitArg - bodyNodeYPosition) ** 2
            ) < 40
        )
            return false;
    }
...
function checkForIntersection() {
    if (body.length >= 4) {
        ...
        for (let i = 0; i < body.length - 4; i++) {
            ...
            if (
                Math.sqrt(
                    (currHeadXPosition - bodyNodeXPosition) ** 2 +
                        (currHeadYPosition - bodyNodeYPosition) ** 2
                ) < 15
            )
                lostScreen();
        }
